...
import {
  ...
  BookOpen, HelpCircle, ArrowRight, CornerDownLeft, RefreshCw
} from 'lucide-react';
...
export default function OpenEndedQuestionView({
  ...
  const renderClozeText = () => {
    ...
    return (
      <div className="leading-relaxed text-base sm:text-lg text-slate-800 my-4 bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-200/80 shadow-inner">
        {parts.map((part, idx) => {
          ...
          return (
            <span key={idx} className="inline-block mx-1 my-1">
              <input
                ...
                style={{ width: `${Math.max(val.length + 3, 10)}ch` }}
              />
              {submitted && !isBlankOk && (
                <span className="ml-1 text-xs font-semibold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded">
                  {blankInfo?.canonico}
                </span>
              )}
            </span>
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-4 sm:py-6">
      ...
      {/* Main Question Card */}
      <Card className="p-5 sm:p-7 rounded-3xl border border-slate-200/80 shadow-xl bg-white mb-6">
        <div className="mb-4">
                ...
                <div key={i} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="w-full sm:w-1/2">
                    <select
                      ...
                      className="w-full px-3 py-2 text-sm sm:text-base rounded-xl border border-slate-300 bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
        ...
        {!submitted && (
          <div className="mt-6 flex justify-end">
            <Button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-7 rounded-2xl shadow-lg shadow-indigo-600/25 transition-all duration-200 active:scale-95"
            >
          ...
          >
            ...
            <Card className="p-5 sm:p-6 rounded-3xl border border-slate-200 bg-white shadow-md">
              <div className="flex items-center gap-2 mb-2 font-bold text-slate-800 text-sm sm:text-base">
                <BookOpen className="w-4 h-4 text-indigo-600" /> Justificación (Moore 9.ª ed.)
              </div>
              ...
              {question?.fuente && (
                <div className="text-xs text-slate-500 pt-2 border-t border-slate-100 flex items-center gap-3">
            ...
            <div className="flex justify-end pt-2">
              <Button
                onClick={onNext}
                className="bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3.5 px-8 rounded-2xl shadow-xl transition-all active:scale-95 flex items-center gap-2 text-base"
              >
